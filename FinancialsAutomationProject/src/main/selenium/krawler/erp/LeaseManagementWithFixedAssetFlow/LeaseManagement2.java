...
public class LeaseManagement2 {
	...
	public static void FixedAssetExpander(WebDriver driver) throws IOException, InterruptedException {
		Properties FA = Utilities.fetchProValue("OR_LeaseManagement.properties");
		Thread.sleep(3000);
		WebDriverWait wait = new WebDriverWait(driver, 30);
		WebElement FAButton = wait
				.until(ExpectedConditions.elementToBeClickable(By.xpath(FA.getProperty("AssetMenubtn"))));
		FAButton.click();
		Thread.sleep(500);

		WebElement Entrybutton = wait
				.until(ExpectedConditions.elementToBeClickable(By.xpath(FA.getProperty("EntryButtonForAsset"))));
		Entrybutton.click();
		Thread.sleep(500);
	}
}
