...
public class DatewiseBatchStockTracking {
	...
	// ********************************************************
	public static void validate_DatewiseBatchStockTracking(String productid, String selectStore, String balanceQty,
			String batch, WebDriver driver) throws InterruptedException, AWTException, IOException {
		...
		try {
			...
			/*
			 * SelectStore.click(); Thread.sleep(1000);
			 * SelectStore.sendKeys(selectStore); Thread.sleep(1000);
			 * SelectStore.sendKeys(Keys.ENTER); Thread.sleep(2000);
			 */
			...
			int tableHeadersCnt = driver
					.findElements(By
							.xpath("//div[@id='datewisebatchInventoryLevel']/div/div/div/div/div/div/div/div[2]/div/div/div/div/div/table/thead/tr/td"))
					.size();
			Thread.sleep(1000);
			...
			for (int j = 1; j <= resultTotalCnt; j++) {
				...
				String productIDStr = driver.findElement(By.xpath("//div[@class='x-grid3-viewport']/div[2]/div/div[" + j
						+ "]/div/div[contains(text(),'Product ID')]")).getText();
			...
			System.out.println(
					"************************************************* 'Datewise Batch Stock Tracking Report' is verified*************************************************");
