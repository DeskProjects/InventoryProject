...
public class AssetSummaryReport {
	public void Generate_AssetSummaryReport(String documentid, WebDriver driver)
		...
		try {
			...
			WebElement ADetailReportbutton = AGwait.until(
					ExpectedConditions.elementToBeClickable(By.xpath(AssetDetailReport.getProperty("ClickOnASR"))));
			ADetailReportbutton.click();
			Thread.sleep(3000);

			/*
			 * // Click On Asset Menu WebElement AMenu =
			...
			WebElement CA = AGwait.until(
					ExpectedConditions.elementToBeClickable(By.xpath(AssetDetailReport.getProperty("CostAdditions"))));
			...
			System.out
					.println("*********Cost Additions  Value Actual Result =  " + ActualResultCA + "    ************");

			if (ExpectedResultCA.equals(ExpectedResultCA) == ActualResultCA.equalsIgnoreCase(ActualResultCA)) {

				System.out
						.println("Matched Cost Additions  Value [" + ExpectedResultCA + "]  [" + ActualResultCA + "]");
	...
	public void Export_AssetASR(String buttonName, WebDriver driver)
			throws InterruptedException, IOException, AWTException {
		...
		try {
			...
			try {
				...
				System.out.println(
						"* * * * * * EXPORT for module [Asset Summary Report] completed successfully * * * * * * *");
			}
